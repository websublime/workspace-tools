//! Package manager detection and handling.
...
impl PackageManager {
    ...
    pub fn detect(path: impl AsRef<Path>) -> StandardResult<Self> {
        ...
        if path.join(PackageManagerKind::Npm.lock_file()).exists()
           || path.join("npm-shrinkwrap.json").exists() // Check both npm lock files
        {
            return Ok(Self::new(PackageManagerKind::Npm, path));
        }

        Err(StandardError::operation(format!(
            "No package manager lock file found in {}", path.display()
        )))
    }
    ...
    pub fn lock_file_path(&self) -> PathBuf {
        ...
        let lock_file = if self.kind == PackageManagerKind::Npm && !self.root.join(PackageManagerKind::Npm.lock_file()).exists() && self.root.join("npm-shrinkwrap.json").exists() {
            "npm-shrinkwrap.json"
        } else {
            self.kind.lock_file()
        };
...
mod tests {
    ...
    fn test_no_package_manager() {
        ...
        match result.unwrap_err() {
            StandardError::Operation(msg) => assert!(msg.contains("No package manager lock file found")),
...
pub struct PackageManager {
    ...
    kind: PackageManagerKind,
    /// Root directory containing package manager files
    root: PathBuf,
}
...
impl PackageManager {
    ...
    /// Detects the package manager used in the given directory by checking for lock files.
    ///
    /// It checks in the order: bun, pnpm, yarn, npm.
    pub fn detect(path: impl AsRef<Path>) -> StandardResult<Self> {
        let path = path.as_ref();
        ...
        // Also check for npm-shrinkwrap.json
        if path.join("npm-shrinkwrap.json").exists() {
            return Ok(Self::new(PackageManagerKind::Npm, path));
        }
...
impl PackageManagerKind {
    ...
    pub fn lock_file(self) -> &'static str {
        ...
        // Return a specific StandardError variant instead of the removed ::new
        Err(StandardError::operation("No package manager detected in specified path"))
    }
=> "bun.lockb",
        }
    }
...
impl PackageManager {
    ...
    /// use sublime_standard_tools::project::PackageManagerKind;
    ///
    /// let pm = PackageManager::new(PackageManagerKind::Npm, "/project/root");
    /// ```
    #[must_use]
    pub fn new(kind: PackageManagerKind, root: impl Into<PathBuf>) -> Self {
        Self {
    ...
    /// # Arguments
    ///
    /// * `path` - Directory to check for package manager files
    ///
    /// # Returns
    ///
    /// Package manager instance if detected
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// let pm = PackageManager::detect("/project/root")?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn detect(path: impl AsRef<Path>) -> StandardResult<Self> {
        ...
        // Check for each package manager's lock file
        if path.join(PackageManagerKind::Npm.lock_file()).exists() {
            return Ok(Self::new(PackageManagerKind::Npm, path));
        }
        if path.join(PackageManagerKind::Yarn.lock_file()).exists() {
        ...
        Err(StandardError::new("No package manager detected"))
    }
...
mod tests {
    ...
    fn test_package_manager_detection() {
        ...
        let npm_lock = root_path.join("package-lock.json");
        File::create(&npm_lock).unwrap();
        let pm = PackageManager::detect(root_path).unwrap();
        assert_eq!(pm.kind(), PackageManagerKind::Npm);
        assert_eq!(pm.root(), root_path);
        assert_eq!(pm.lock_file_path(), npm_lock);
    }

    #[test]
    fn test_no_package_manager() {
        let temp_dir = TempDir::new().unwrap();
        assert!(PackageManager::detect(temp_dir.path()).is_err());
    }
}
