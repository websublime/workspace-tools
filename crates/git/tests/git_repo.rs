...
mod repo_tests {
    ...
    #[test]
    fn test_add_repo() -> Result<(), RepositoryError> {
        let monorepo_root_dir = create_monorepo()?;
        ...
        execute(
            "git",
            monorepo_root_dir.as_path(),
            ["commit", "-m", "chore: add single file"],
            |_, stdout| Ok(stdout.status.success()),
        )?;

        let logs = repo.log(Some(String::from("main..HEAD")))?;

        assert!(logs.contains("chore: add single file"));
        assert!(added);
    ...
    #[test]
    fn test_current_sha_repo() -> Result<(), RepositoryError> {
        let monorepo_root_dir = create_monorepo()?;
    ...
    #[test]
    fn test_previous_sha_repo() -> Result<(), RepositoryError> {
        let monorepo_root_dir = create_monorepo()?;
    ...
    #[test]
    fn test_diverged_commit_repo() -> Result<(), RepositoryError> {
        let monorepo_root_dir = create_monorepo()?;
        ...
        let sha = repo.get_diverged_commit("main")?;
    ...
    #[test]
    fn test_status_repo() -> Result<(), RepositoryError> {
        let monorepo_root_dir = create_monorepo()?;
    ...
    fn test_commit_repo() -> Result<(), RepositoryError> {
        ...
        assert!(commited);
        assert!(logs.contains("chore: add main.mjs file"));
    ...
    #[test]
    fn test_all_files_changed_since_sha_repo() -> Result<(), RepositoryError> {
        let monorepo_root_dir = create_monorepo()?;
