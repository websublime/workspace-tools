...
pub fn generate_dot<N: Node>(graph: &DependencyGraph<N>, options: &DotOptions) -> Result<String> {
    ...
    for node_idx in graph.graph.node_indices() {
        ...
        match node {
            crate::graph::node::Step::Resolved(package) => {
            ...
            crate::graph::node::Step::Unresolved(_dependency) => {
    ...
    for edge_idx in graph.graph.edge_indices() {
        if let Some((source_idx, target_idx)) = graph.graph.edge_endpoints(edge_idx) {
            ...
            let source_id = match &graph.graph[source_idx] {
                crate::graph::node::Step::Resolved(package) => package.identifier().to_string(),
                crate::graph::node::Step::Unresolved(_) => {
    ...
    for component in scc {
        if component.len() > 1 {
            ...
            for &node_idx in &component {
                if let crate::graph::node::Step::Resolved(node) = &graph.graph[node_idx] {
...
/// ```
pub fn save_dot_to_file(dot_content: &str, file_path: &str) -> Result<()> {
    std::fs::write(file_path, dot_content).map_err(|e| PkgError::IoError {
        path: Some(std::path::PathBuf::from(file_path)),
        source: e,
    })
}
...
/// println!("{}", ascii);
/// ```
pub fn generate_ascii<N: Node>(graph: &DependencyGraph<N>) -> Result<String> {
    let mut output = String::new();
    ...
    for node_idx in graph.graph.node_indices() {
        let incoming =
            graph.graph.neighbors_directed(node_idx, petgraph::Direction::Incoming).count();
        if incoming == 0 {
            if let crate::graph::node::Step::Resolved(node) = &graph.graph[node_idx] {
...
fn add_ascii_children<N: Node>(
    ...
    for child_idx in graph.graph.neighbors_directed(node_idx, petgraph::Direction::Outgoing) {
        match &graph.graph[child_idx] {
            crate::graph::node::Step::Resolved(node) => {
