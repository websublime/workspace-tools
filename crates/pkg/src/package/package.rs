...
/// let dep1 = Rc::new(RefCell::new(Dependency::new("react", "^17.0.2")?));
...
impl Package {
    ...
    /// # fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// // Create dependency
    /// let dep = Rc::new(RefCell::new(Dependency::new("react", "^16.0.0")?));
    ///
    /// // Create package with the dependency
    /// let pkg = Package::new("my-app", "1.0.0", Some(vec![Rc::clone(&dep)]))?;
    ///
    ...
    ///
    /// // Add a dependency
    /// let dep = Rc::new(RefCell::new(Dependency::new("react", "^17.0.0")?));
