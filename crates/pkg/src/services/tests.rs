...
mod tests {
    ...
    mod unit_tests {
        ...
        async fn test_monorepo_optimization() {
            ...
            let monorepo_config = MonorepoContext {
                workspace_packages,
                ..Default::default()
            };
        ...
        async fn test_monorepo_size_scaling() {
            ...
            let sizes_and_expected = vec![
                ...
                (80, 3, 20),    // Large: 80 packages -> 3 concurrent, 20 batch
                (150, 3, 20),   // Very large: 150 packages -> 3 concurrent, 20 batch
            ];
            ...
            for (package_count, expected_concurrent, expected_batch) in sizes_and_expected {
                ...
                assert_eq!(
                    ...
                    "Wrong concurrent downloads for {} packages", package_count
                );
                assert_eq!(
                    strategy.batch_processing_size, expected_batch,
                    "Wrong batch size for {} packages", package_count
                );
            }
        }
        ...
        async fn test_error_propagation() {
            ...
            let items = vec![1, 2, 3, 4, 5];
            let result = processor.process_concurrent(
                tokio_stream::iter(items),
                |x| async move {
                    if x == 3 {
                        Err("Error on item 3")
        ...
        async fn test_custom_concurrency() {
            ...
            let items = vec![1, 2, 3, 4, 5];
            let results = processor.process_with_custom_concurrency(
                tokio_stream::iter(items),
                |x| async move {
                    sleep(Duration::from_millis(10)).await;
        ...
        async fn test_specialized_network_processor() {
            ...
            let results = network_processor.process_requests(
                tokio_stream::iter(items),
                |url| async move {
        ...
        async fn test_specialized_filesystem_processor() {
            ...
            let results = fs_processor.process_files(
                tokio_stream::iter(items),
                |filename| async move {
    ...
    mod integration_tests {
        ...
        async fn test_package_service_with_optimization() {
            ...
            let service = PackageService::with_performance_optimization(
                fs.clone(), 
                context
            ).await.unwrap();
        ...
        async fn test_package_service_fallback_without_optimization() {
            ...
            let packages = vec![
                Package::new("test", "1.0.0", None).unwrap(),
            ];
            
            // Should still work, just without optimization
            let results = service.process_packages_concurrent(
                packages,
                |pkg| async move { Ok(pkg) }
            ).await.unwrap();
        ...
        async fn test_package_service_version_updates_concurrent() {
            ...
            let updates = vec![
                (Package::new("pkg1", "1.0.0", None).unwrap(), "1.1.0", std::path::Path::new("pkg1/package.json")),
                (Package::new("pkg2", "1.0.0", None).unwrap(), "1.2.0", std::path::Path::new("pkg2/package.json")),
                (Package::new("pkg3", "1.0.0", None).unwrap(), "1.3.0", std::path::Path::new("pkg3/package.json")),
            ];
            
            let updated_packages = service.update_package_versions_concurrent(updates).await.unwrap();
        ...
        async fn test_command_service_with_optimization() {
            ...
            let service = PackageCommandService::with_performance_optimization(
                executor,
                fs,
                context
            ).await.unwrap();
            ...
            assert!(service.performance_optimizer.is_some());
            assert!(service.concurrent_processor.is_some());
        }
        ...
        async fn test_command_service_custom_strategy() {
            ...
            let service = PackageCommandService::with_custom_strategy(
                executor,
                fs,
                strategy
            );
            ...
            assert!(service.performance_optimizer.is_none());
        }
    }
    ...
    mod performance_tests {
        ...
        async fn benchmark_concurrent_vs_sequential() {
            ...
            let results_conc = processor.process_concurrent(
                tokio_stream::iter(items),
                |x| async move {
                    sleep(Duration::from_millis(10)).await;
                    Ok::<i32, Error>(x * 2)
            ...
            println!("Sequential: {:?}, Concurrent: {:?}, Speedup: {:.2}x",
                duration_seq, duration_conc,
        ...
        async fn test_concurrent_service_access() {
            ...
            let context = ProjectContext::Single(SingleRepositoryContext::default());
            let service = Arc::new(
                PackageService::with_performance_optimization(fs, context).await.unwrap()
            );
            ...
            for i in 0..10 {
                ...
                tasks.spawn(async move {
                    let packages = vec![
                        Package::new(&format!("pkg-{}", i), "1.0.0", None).unwrap()
                    ];
                    
                    service_clone.process_packages_concurrent(
                        packages,
                        |mut pkg| async move {
        ...
        async fn test_optimization_strategy_bounds() {
            ...
            let contexts = vec![
                ...
                ProjectContext::Monorepo(MonorepoContext {
                    workspace_packages: (0..200).map(|i| (format!("p{}", i), format!("pkg/{}", i))).collect(),
                    ..Default::default()
                }),
            ];
