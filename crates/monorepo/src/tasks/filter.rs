...
impl TaskFilter {
    ...
    pub fn apply(&self, tasks: &[Task]) -> TaskResult<Vec<Task>> {
        ...
        let include_globs = self.build_glob_set(&self.include)?;
        let exclude_globs = self.build_glob_set(&self.exclude)?;
        ...
        if self.include_dependencies || self.include_dependents {
            ...
            for task in tasks {
                if dependencies_to_add.contains(&task.name) && !included_task_names.contains(&task.name) {
                    filtered_tasks.push(task.clone());
                    included_task_names.insert(task.name.clone());
                }
            }
    ...
    fn matches_task(&self, task: &Task, include_globs: &GlobSet, exclude_globs: &GlobSet) -> bool {
        ...
        let should_include = self.include.len() == 0 || include_globs.is_match(&task.name);
        
        // Exclude patterns override include patterns
        let should_exclude = self.exclude.len() > 0 && exclude_globs.is_match(&task.name);
        
        should_include && !should_exclude
    }
    
    // Helper method to build a GlobSet from patterns
    fn build_glob_set(&self, patterns: &[String]) -> TaskResult<GlobSet> {
        ...
        match builder.build() {
            Ok(set) => Ok(set),
            Err(err) => Err(TaskError::FilterError(format!(
                "Failed to build glob set: {}", err
            ))),
        }
    }
    
    // Helper to recursively collect all dependencies
    fn collect_dependencies(&self, all_tasks: &[Task], dependencies: &mut HashSet<String>, task: &Task) {
