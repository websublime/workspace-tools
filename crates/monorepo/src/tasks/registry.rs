...
impl TaskRegistry {
    ...
    pub fn get_package_script_tasks(&self) -> Vec<&TaskDefinition> {
        self.tasks
            .values()
            .filter(|task| !task.package_scripts.is_empty())
            .collect()
    }
        ...
        self.tasks
            ...
            .filter(|task| {
                match &task.scope {
                    ...
                    _ => task.package_scripts.iter().any(|script| {
                        script.package_name.as_deref() == Some(package_name)
                    }),
                }
            })
            .collect()
    ...
    fn validate_task(&self, task: &TaskDefinition) -> Result<()> {
        ...
        for dependency in &task.dependencies {
            if !self.tasks.contains_key(dependency) && dependency != &task.name {
                return Err(Error::task(format!(
                    "Task dependency not found: {dependency}"
                )));
            }
        }
        ...
        if task.dependencies.contains(&task.name) {
            return Err(Error::task(format!(
                "Task cannot depend on itself: {}", task.name
            )));
        }
        ...
        if let Some(task) = self.tasks.get(task_name) {
            for dependency in &task.dependencies {
                if !visited.contains(dependency) {
                    ...
                    // Found cycle
                    let cycle_start = path.iter().position(|t| t == dependency).unwrap();
