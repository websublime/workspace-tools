...
impl TaskGraph {
    ...
    pub fn from_tasks(tasks: &[Task]) -> TaskResult<Self> {
        ...
        for task in tasks {
            let from_idx = *node_indices.get(&task.name).unwrap();
            ...
            for dep_name in &task.dependencies {
                ...
                let to_idx = *node_indices.get(dep_name).unwrap();
        ...
        if let Err(cycle) = algo::toposort(&graph, None) {
            ...
            while let Some(task_name) = stack.pop() {
                ...
                if stack.len() == 0 || cycle_str.find(&cycle_task).is_some() {
                    break;
                }
            }
    ...
    pub fn sorted_tasks(&self, mode: TaskSortMode) -> TaskResult<Vec<Task>> {
        match mode {
            TaskSortMode::Topological => self.topological_sort(),
            TaskSortMode::Parallel => self.parallel_sort(),
            TaskSortMode::Random => self.random_sort(),
        }
    }
    ...
    pub fn task_levels(&self) -> Vec<Vec<Task>> {
        ...
        for (task_name, _) in &self.tasks {
            let idx = *self.node_indices.get(task_name).unwrap();
        ...
        while visited.len() < self.tasks.len() {
            ...
            for (task_name, _) in &self.tasks {
                ...
                let task_idx = *self.node_indices.get(task_name).unwrap();
                let dependencies = self.graph.neighbors_directed(task_idx, Direction::Outgoing)
    ...
    fn topological_sort(&self) -> TaskResult<Vec<Task>> {
    ...
    fn parallel_sort(&self) -> TaskResult<Vec<Task>> {
    ...
    fn random_sort(&self) -> TaskResult<Vec<Task>> {
