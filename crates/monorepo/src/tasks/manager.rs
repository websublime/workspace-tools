...
use super::{
    TaskDefinition, TaskExecutionResult, TaskScope, TaskRegistry,
...
impl TaskManager {
    ...
    pub async fn execute_task(
        ...
        let task_definition = self.registry.get_task(task_name)
        ...
        if !self.condition_checker.check_conditions(&task_definition.conditions).await? {
            return Ok(TaskExecutionResult::new(task_name).with_status_skipped(
                "Conditions not met"
            ));
        }
    ...
    pub async fn execute_tasks_for_changes(
        ...
        let affected_packages: Vec<String> = changes.package_changes
            .iter()
    ...
    pub async fn execute_tasks_batch(
        ...
        for task_name in execution_order {
            ...
            if result.is_failure() {
                let task = self.registry.get_task(&task_name)
                    .ok_or_else(|| Error::task(format!("Task not found: {task_name}")))?;
    ...
    pub async fn resolve_package_tasks(
        &self,
        package_name: &str,
        ...
        if let Some(scripts) = package_json.get("scripts").and_then(|s| s.as_object()) {
            for (script_name, _script_command) in scripts {
                let task = TaskDefinition::new(
                    ...
                    super::PackageScript::new(script_name)
    ...
    async fn execute_task_with_context(
        ...
        if !self.condition_checker.check_conditions_with_context(
            &task.conditions,
            context
        ).await? {
            return Ok(TaskExecutionResult::new(&task.name).with_status_skipped(
                "Conditions not met"
            ));
        }
    ...
    async fn get_tasks_for_changes(
        &self,
        changes: &ChangeAnalysis,
    ) -> Result<Vec<TaskDefinition>> {
...
impl TaskExecutionResult {
    ...
    pub fn with_status_skipped(mut self, reason: impl Into<String>) -> Self {
        self.status = super::TaskStatus::Skipped {
            reason: reason.into(),
        };
        self
    }
