...
fn create_test_package_script_task() -> TaskDefinition {
    TaskDefinition::new(
        "test-script-task",
        "A test task with package scripts"
    )
    .with_package_script(
        PackageScript::new("build")
            .for_package("test-package-a")
    )
...
fn create_test_conditional_task() -> TaskDefinition {
    TaskDefinition::new(
        "conditional-task",
        "A task that runs conditionally"
    )
    .with_command(
        TaskCommand::new("echo")
            .with_args(vec!["Conditional execution".to_string()])
    )
...
mod task_definition_tests {
    ...
    fn test_task_builder_pattern() {
        let task = TaskDefinition::new("builder-test", "Testing builder pattern")
            ...
            .with_condition(TaskCondition::OnBranch {
                pattern: BranchCondition::IsFeature,
            });
...
mod task_registry_tests {
    ...
    fn test_tasks_by_scope() {
        ...
        let global_task = TaskDefinition::new("global-task", "Global task")
            .with_scope(TaskScope::Global);
        ...
        let package_tasks = registry.get_tasks_for_scope(
            &TaskScope::Package("test-package".to_string())
        );
    ...
    fn test_tasks_by_priority() {
        ...
        let low_task = TaskDefinition::new("low-task", "Low priority task")
...
mod task_execution_tests {
    ...
    fn test_task_execution_result_status_updates() {
        ...
        failed_result.add_error(TaskError::new(
...
mod condition_checker_tests {
    ...
    fn test_packages_changed_condition() {
        ...
        let condition = TaskCondition::PackagesChanged {
            packages: vec!["test-package-a".to_string()],
        };
    ...
    fn test_branch_condition() {
        ...
        let condition = TaskCondition::OnBranch {
            pattern: BranchCondition::IsFeature,
        };
    ...
    fn test_not_condition() {
        ...
        let not_condition = TaskCondition::Not {
            condition: Box::new(TaskCondition::OnBranch {
                pattern: BranchCondition::IsFeature,
            }),
        };
    ...
    fn test_check_packages_changed() {
        ...
        let result = run_async(checker.check_packages_changed(&["package-a".to_string()], &context));
        assert!(result.is_ok());
        assert!(result.unwrap(), "Should detect affected package");
        
        let result = run_async(checker.check_packages_changed(&["package-c".to_string()], &context));
        assert!(result.is_ok());
    ...
    fn test_check_environment_condition_variable_exists() {
        ...
        let condition = EnvironmentCondition::VariableExists {
        ...
        let condition = EnvironmentCondition::VariableExists {
            key: "NON_EXISTENT_VAR".to_string(),
        };
    ...
    fn test_check_branch_condition_is_main() {
        ...
        for (branch_name, expected) in test_cases {
            ...
            let result = run_async(checker.check_branch_condition(&condition, &context));
            assert!(result.is_ok());
            assert_eq!(result.unwrap(), expected, "Branch '{}' should be {}", branch_name, expected);
        }
    }
    ...
    fn test_task_matches_changes() {
        ...
        let task = TaskDefinition::new("test-task", "Test task")
            .with_condition(TaskCondition::PackagesChanged {
                packages: vec!["test-package-a".to_string()],
            });
