...
# Hook strategy: "auto", "native", "husky", or "hybrid"
strategy = "auto"
...
package_manager = "npm"  # "npm", "yarn", "pnpm", "bun", or null for auto-detect
...
init_command = "husky install"

# Auto-detection settings
[hooks.auto_detection]
...
### 🪝 Hooks Configuration with Husky Support

Advanced Git hooks with intelligent strategy detection and Husky integration:

- **`enabled`**: Whether hooks are globally enabled
- **`strategy`**: Hook implementation strategy (auto, native, husky, hybrid)
- **`hooks_dir`**: Directory for native Git hook scripts
...
- **`husky`**: Husky-specific configuration and package manager integration
- **`auto_detection`**: Intelligent detection settings for optimal hook strategy

## 🎣 Advanced Hook Strategies & Husky Integration

### Hook Strategies

The monorepo tools support four different hook strategies:
...
strategy = "auto"
```
- **Intelligent Detection**: Automatically analyzes your project setup
- **Node.js Projects**: Prefers Husky for better ecosystem integration
- **Non-Node.js Projects**: Uses native Git hooks for simplicity
- **Existing Hooks**: Respects and maintains existing configurations
...
strategy = "native"
```
- **Traditional Git Hooks**: Uses `.git/hooks/` directory
- **Universal Compatibility**: Works with any project type
- **Simple Setup**: No additional dependencies required
- **Direct Execution**: Runs commands directly without package manager integration
...
strategy = "husky"
```
- **Node.js Optimized**: Perfect for npm/yarn/pnpm projects
- **Modern Workflow**: Uses `.husky/` directory structure
- **Package Scripts**: Integrates with package.json scripts
- **Team Friendly**: Easier to share and version control
...
strategy = "hybrid"
```
- **Best of Both**: Combines native Git hooks and Husky
- **Maximum Compatibility**: Works across different team setups
- **Gradual Migration**: Perfect for transitioning between systems
- **Fail-safe**: Continues working if one system fails
...
#### Advanced Husky with Custom Package Manager
```toml
[hooks]
strategy = "husky"
...
package_manager_args = ["--filter", "changed"]
use_package_scripts = true
init_command = "pnpm exec husky install"

[hooks.pre_push]
enabled = true
run_tasks = ["test:affected", "build:affected"]
```

#### Hybrid Setup for Mixed Teams
```toml
[hooks]
strategy = "hybrid"

# Configure both native and Husky hooks
[hooks.pre_commit]
enabled = true
run_tasks = ["lint"]
custom_script = "./scripts/pre-commit.sh"  # Fallback for non-Husky users
...
#### From Native Git Hooks to Husky
```bash
# 1. Install Husky in your project
npm install husky --save-dev

# 2. Update monorepo.toml
[hooks]
strategy = "auto"  # Will detect Husky and prefer it

[hooks.auto_detection]
prefer_husky = true

# 3. Install hooks
# The tool will automatically create Husky hooks
```

#### From Husky to Native (CI/CD environments)
```toml
[hooks]
strategy = "native"

# Keep same task configuration
[hooks.pre_commit]
enabled = true
run_tasks = ["lint", "test"]
```
...
```bash
# Generated .husky/pre-commit
pnpm run lint --filter changed
pnpm run test --filter changed
```
